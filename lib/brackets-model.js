...
/** Format d’un match : BO1, BO3 ou BO5. */
const ALLOWED_MATCH_BEST_OF = [1, 3, 5];
...
function normalizeOneGroup(g, idx) {
  ...
  return {
    id: String(g.id || ('group_' + idx)).trim(),
    title: String(g.title || ('Groupe ' + String.fromCharCode(65 + idx))).trim(),
    rounds
  };
}

function normalizeGroupsTournament(t) {
  const groups = (t.groups || []).map((g, i) => normalizeOneGroup(g, i));
  return {
    id: String(t.id || 'groups').trim() || 'groups',
    title: String(t.title || 'Phase de groupes').trim(),
    description: String(t.description || '').trim(),
    type: 'groups',
    groups
  };
...
function normalizeTournament(t) {
  ...
  if (t.type === 'groups') return normalizeGroupsTournament(t);
...
function getMatchRef(tournament, lane, roundIndex, matchIndex) {
  ...
  if (lane.startsWith('groups:')) {
    const groupId = lane.slice('groups:'.length);
    const grp = (tournament.groups || []).find((g) => g.id === groupId);
    if (!grp || !grp.rounds || !grp.rounds[roundIndex]) return null;
    return (grp.rounds[roundIndex].matches || [])[matchIndex] || null;
  }
...
function applyComputedSeriesWinner(match, winnerByChecksum) {
  ...
  /* Ne pas effacer un vainqueur manuel tant que la série n’est pas mathématiquement décidée (ex. BO3 avec 1 démo). */
  if (w != null) match.winner = w;
}
...
/**
 * Pour chaque checksum présent dans un match BO3 (au moins 2 démos), renvoie la liste ordonnée
 * des démos et le bestOf — utilisé par l’API stats pour l’overlay série.
 * @param {{ tournaments?: unknown[] }} v2Payload
 * @returns {Map<string, { demoIds: string[], bestOf: number }>}
 */
function buildSeriesDemoLookup(v2Payload) {
  const map = new Map();
  const tournaments = v2Payload && Array.isArray(v2Payload.tournaments) ? v2Payload.tournaments : [];
  const walk = (m) => {
    ...
    if (!demoIds.length && m.demoId != null && String(m.demoId).trim() !== '') {
      demoIds.push(String(m.demoId).trim());
    }
    const bestOf = normalizeMatchBestOf(m.bestOf);
    const minDemos = bestOf >= 3 ? 2 : 0;
    if (bestOf >= 3 && demoIds.length >= minDemos) {
      demoIds.forEach((id) => map.set(id, { demoIds: demoIds.slice(), bestOf }));
    }
  };
  for (const t of tournaments) {
    if (!t || typeof t !== 'object') continue;
    if (t.type === 'swiss' && Array.isArray(t.rounds)) {
      t.rounds.forEach((r) => (r.matches || []).forEach(walk));
    } else if (t.type === 'groups' && Array.isArray(t.groups)) {
      t.groups.forEach((g) => (g.rounds || []).forEach((r) => (r.matches || []).forEach(walk)));
    } else if (t.type === 'elimination') {
      (t.upperRounds || []).forEach((r) => (r.matches || []).forEach(walk));
      (t.lowerRounds || []).forEach((r) => (r.matches || []).forEach(walk));
      if (t.grandFinale && Array.isArray(t.grandFinale.matches)) {
        t.grandFinale.matches.forEach(walk);
      }
    }
  }
  return map;

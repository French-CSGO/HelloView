...
/** Format d’un match : BO1 (une démo) ou BO3 (1 à 3 démos au fil du tournoi, 2 manches pour gagner). */
...
function buildSeriesDemoLookup(v2Payload) {
  ...
  const walk = (m) => {
    ...
    if (bestOf === 3 && demoIds.length >= 2) {
      demoIds.forEach((id) => map.set(id, { demoIds: demoIds.slice(), bestOf: 3 }));
    }
  };

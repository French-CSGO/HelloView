  ...
  for (const ent of entries) {
    if (ent.isDirectory()) {
      ...
      for (const f of files) {
        if (!f.isFile() || !/\.(dem|zip)$/i.test(f.name)) continue;
        const key = f.name.toLowerCase();
        if (!map.has(key)) map.set(key, { server: ent.name, file: f.name });
      }
    } else if (ent.isFile() && /\.(dem|zip)$/i.test(ent.name)) {
...
function demoDownloadForDbPath(index, demoPath) {
  ...
  if (!base) return null;
  let entry = index.get(base.toLowerCase());
  // Fallback: if DB has .dem but file is .zip (or vice-versa), try alternate extension
  if (!entry) {
    const alt = base.toLowerCase().endsWith('.dem')
      ? base.slice(0, -4) + '.zip'
      : base.toLowerCase().endsWith('.zip')
        ? base.slice(0, -4) + '.dem'
        : null;
    if (alt) entry = index.get(alt);
  }
